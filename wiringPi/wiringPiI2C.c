...
int wiringPiI2CWrite(int fd, int data)
{
	...
	{
		LogFormatted(LogLevelError, "wiringPiI2C.c", "wiringPiI2CWrite", "Error writing fd %d data %d. Error code %d.", fd, data, ret);
	}
	return ret;
}
...
int wiringPiI2CWriteReg8(int fd, int reg, int value)
{
	...
	{
		LogFormatted(LogLevelError, "wiringPiI2C.c", "wiringPiI2CWriteReg8", "Error writing fd %d register 0x%x data %d. Error code %d.", fd, reg, data.byte, ret);
	}
	return ret;
}
...
int wiringPiI2CWriteReg16(int fd, int reg, int value)
{
	...
	{
		LogFormatted(LogLevelError, "wiringPiI2C.c", "wiringPiI2CWriteReg16", "Error writing fd %d register 0x%x data %d. Error code %d.", fd, reg, data.word, ret);
	}
	return ret;
}
...
int wiringPiI2CSetupInterface(const char* device, int devId)
{
	...
	LogFormatted(LogLevelInfo, "wiringPiI2C.c", "wiringPiI2CSetupInterface", "Setup I2C interface %s 0x%x. File Descriptor (fd) %d.", device, devId, fd);
	return fd;
}
